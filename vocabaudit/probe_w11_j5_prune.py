"""W11-J5 (prereg_w11_j5_vocabprune.md): detector-guided vocabulary pruning
for dense CLIP. 9 dense cells + synonym-harm control."""
import argparse
import json
...
if __name__ == "__main__":
    ...
    res = {"prereg": "prereg_w11_j5_vocabprune.md"}
    cells = [(v, ds, f"perturbed_vocabs/{ds}_plain.json")
             for v in ("sclip", "clearclip", "naclip")
             for ds in ("voc21", "ctx60", "cocoobj")]
    cells.append(("sclip", "voc21", "perturbed_vocabs/voc21_syn100_s0.json"))
    for v, ds, vf in cells:
        base, pruned = cell(v, ds, vf, proc, owl)
        key = f"{v}/{ds}/{vf.split('/')[-1]}"
