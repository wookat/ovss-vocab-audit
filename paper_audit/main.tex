...
\paragraph{Training-free OVSS.} MaskCLIP \citep{zhou2022maskclip} extracts dense
...
template ensembles \citep{radford2021clip}, LLM-generated descriptions
...
the names of a fixed target class, which is the axis audited here. To our knowledge no prior work performs a controlled multi-axis
...
\paragraph{Frozen inference protocol.} OpenCLIP ViT-B/16 (OpenAI weights, QuickGELU);
the 80 ImageNet prompt templates; short side 336, sliding window 224, stride 112, mean
...
\miou{} is computed over classes present in ground truth. No PAMR or other
post-processing. Under this protocol our SCLIP reproduction reaches 55.4 \miou{} on
...
WordNet noun-synset lemmas filtered by CLIP text cosine similarity to the original name
...
reproductions): ProxyCLIP-style DINO proxy attention, LPOSS-style DINO-affinity
...
(CLIP$+$DINO feature splicing with SAM-encoder aggregation and SAM
...
release (ICCV~2025 oral; MetaCLIP + DINO + the authors' pre-generated SAM2
region masks) reproduces its published scores to within 0.1 (VOC 74.79 vs
...
prompting SAM with their boxes (Grounded-SAM-style harness; detection scores
arbitrate overlapping masks; unclaimed pixels are background). We audit
Grounding DINO (BERT-based phrase grounding) and OWLv2 (CLIP-initialised text
...
falsifies lineage as the organising variable: OWL-ViT~v1, which shares
